...
const VerifyOtp = () => {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/auth/otp', { otp, userId: user?._id });
  ...
  const handleSendOtp = async () => {
    ...
    try {
      setDisabled(true);
      const response = await axios.post('http://localhost:8000/api/auth/sendotp', {
        userId: user._id,
        email: user?.email,
      });
