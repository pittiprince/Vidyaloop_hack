...
export default function Login() {
    ...
    const {setUser, user, setUserId} = context
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Changed from GET to POST and fixed how parameters are passed
            const response = await axios.post("http://localhost:8000/api/teacher-auth/signin", {
            ...
            if(data?.success === 1) {
                if(!data.user.isValid) {
                    ...
                    localStorage.setItem("tracker-email-teacher", data.user.email);
                    localStorage.setItem("tracker-id-teacher", data.user._id);
                    
                    // Pass the user ID and email directly to handleSendOtp
                    handleSendOtp(data.user._id, data.user.email);
                    
                    setLoader(false);
                    navigate("/otp");
                } else {
                    localStorage.setItem("tracker-token-teacher", data.token);
                    localStorage.setItem("tracker-name-teacher", data.user.name);
                    localStorage.setItem("tracker-email-teacher", data.user.email);
                    localStorage.setItem("tracker-id-teacher", data.user._id);
